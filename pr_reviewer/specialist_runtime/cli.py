...
_ROLE_SYSTEM = {
    ...
    "critic": (
        ...
        " For an external contract, check that the source establishes the same API or product, "
        "operation, version where relevant, credential type, and permission named in the claim. "
        "Similar terminology is not proof: artifact metadata permissions do not establish "
        "workflow artifact upload permissions. Search snippets are discovery only, even from "
        "official allowlisted hosts; require fetched source evidence. A wrapper calling an SDK "
        "does not by itself establish the SDK's endpoint or authentication mechanism. Reject "
        "unsupported external premises rather than accepting a plausible-sounding execution path."
        " Consider candidate_assessments from the latest collector checkpoints, especially "
        "qualifications that undermine an earlier candidate. These are context, not proof or "
        "automatic withdrawals. truncated describes the retained source; excerpt_truncated "
        "describes what you can actually see. Missing text in a shortened excerpt proves "
        "neither presence nor absence of the claimed contract."
    ...
    "handoff_summarizer": (
        ...
        "For Human focus, translate unresolved_assessments reasons into the concrete "
...
_SPECIALIST_SYSTEM = (
    ...
    "requirement. First establish the actual invoked API and "
    "authentication mechanism from evidence. Ask helpers which mechanism is used and "
    "what the source establishes, not to find a sentence confirming your suspected rule. "
    "Treat controller source_metadata as authoritative for truncation even when helper "
    "prose disagrees; missing information alone is not a truncated result. "
    "A source that does not establish the premise is not proof by analogy "
    ...
    "do not wait for a checkpoint. If later evidence disproves it or a necessary premise "
    "remains unsupported, call "
