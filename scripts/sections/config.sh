...
TOOL_MAX_ROUNDS="${TOOL_MAX_ROUNDS:-3}"
TOOL_LOOP_WALL_CLOCK_SEC="${TOOL_LOOP_WALL_CLOCK_SEC:-120}"
TOOL_SYNTHESIS_TIMEOUT_SEC="${TOOL_SYNTHESIS_TIMEOUT_SEC:-60}"
TOOL_SYNTHESIS_MAX_TOKENS="${TOOL_SYNTHESIS_MAX_TOKENS:-2048}"
TOOL_LOOP_SUMMARIZE_MAX_TOKENS="${TOOL_LOOP_SUMMARIZE_MAX_TOKENS:-512}"
...
SPECIALIST_CHECKPOINT_REASONING_BUDGET_TOKENS="${SPECIALIST_CHECKPOINT_REASONING_BUDGET_TOKENS-}"
...
SPECIALIST_MAX_TRUNCATION_CONTINUATIONS="${SPECIALIST_MAX_TRUNCATION_CONTINUATIONS:-2}"
...
for _positive_budget_name in \
  TOOL_MAX_REQUESTS TOOL_MAX_ROUNDS TOOL_LOOP_WALL_CLOCK_SEC \
  TOOL_SYNTHESIS_TIMEOUT_SEC TOOL_SYNTHESIS_MAX_TOKENS \
  TOOL_PLANNING_TIMEOUT_SEC TOOL_PLANNING_MAX_CONTEXT_BYTES \
  TOOL_PLANNING_MAX_TOKENS TOOL_MAX_RESPONSE_BYTES \
  TOOL_LOOP_SUMMARIZE_MAX_TOKENS TOOL_REQUEST_TIMEOUT_SEC; do
  _positive_budget_value="${!_positive_budget_name}"
  if [[ ! "$_positive_budget_value" =~ ^[1-9][0-9]*$ ]]; then
    error "Invalid ${_positive_budget_name} '${_positive_budget_value}'; expected a positive integer"
    exit 1
  fi
done
...
for _specialist_budget_name in \
  ...
  SPECIALIST_PASS_TIMEOUT_SEC SPECIALIST_MAX_TOKENS SPECIALIST_RECOVERY_MAX_TOKENS \
  SPECIALIST_MAX_CONVERSATION_TOKENS SPECIALIST_MAX_TRUNCATION_CONTINUATIONS \
...
case "$SPECIALIST_STREAM_WATCHDOG" in
  true|false) ;;
  *)
    error "Invalid SPECIALIST_STREAM_WATCHDOG '$SPECIALIST_STREAM_WATCHDOG'; expected true or false"
    exit 1 ;;
esac
