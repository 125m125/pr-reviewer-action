...
| `specialist_delegated_summary_max_source_bytes` | added |  | Leave blank initially | Shared across all sources in a delegated summary, not per source. Derives a cap from the real context window after prompt, initial response, repair, and safety reserves; set only when provider measurements justify a tighter ceiling. |
