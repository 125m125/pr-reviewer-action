...
Before synthesis and verdict, older tool results are compacted while recent evidence remains detailed. The final strict request uses the full review corpus as space permits plus a concise synthesis memo and deterministic evidence ledger, reserving configured verdict-output space under `model_context_tokens`. The step summary and `tool-harness.json` expose configured/effective budgets, turn finish reasons, text sources, call dispositions, remaining allowance, synthesis status, and compaction telemetry. A useful voluntary no-tool analysis can serve as the memo; a blank no-tool turn still degrades safely to the corpus-only review. Loop turns stream by default (`ai_stream`). Supported tools are:
